...
def show_sidebar(data):
    with st.sidebar:
        ...
        for icon_char, label, key in NAV:
            btn_label = f"{icon_char}  {label}"
...
def page_settings(data, save_fn):
    ...
    with c_email:
        if st.button("Send Test Email (demo)", key="send_test_email"):
